...
from keras.layers import Dense, Input, Flatten, Conv2D, MaxPooling2D, Embedding, CuDNNLSTM, LSTM, Activation, RepeatVector, Permute
import random
...
class DQN():
    ...
    def create_model(self):
        ...
        if self.state_mode == "information":
            ...
            out = Embedding(10,2)(input)
            out = CuDNNLSTM(12, return_sequences=False)(out)
            out = Activation('softmax')(out)
            out = RepeatVector(12)(out)
            out = Permute([2, 1])(out)
            out = CuDNNLSTM(12, return_sequences=False)(out)
    ...
    def save_info(self, episode, reward, time):
        file = open("./Plot/DQNwAttention-"+self.game+"-"+str(self.num_of_episodes)+"-episodes-batchsize-"+str(self.batch_size), 'a')
